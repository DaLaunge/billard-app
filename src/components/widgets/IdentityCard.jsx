...
import { initials, fmtDate, decayStatus } from "../../lib/format";
...
/* Vereinheitlichte Identitaets-Karte: grosses, zentriertes Foto im Fokus,
   ...
   unberuehrt. Der Punkteverfall-Hinweis (DecayBadge, siehe widgets/
   DecayBadge.jsx) spiegelt das an der oberen linken Ecke - selber Grund.
   pulse=true, weil hier immer genau eine (die betroffene) Person im Fokus
   steht - im Unterschied zur Rangliste, wo mehrere Treffer gleichzeitig
   zu sehen sein koennen (siehe RanglisteScreen.jsx). */
export default function IdentityCard({ nickname, gesamt, motto, since, stats, colorOf, badgeOf, photoOf,
  ...
  const decay = decayStatus(gesamt?.letzte_partie);
  const head = (
    <>
  ...
  return (
    <section className="stat-block id-card">
      ...
      <DecayBadge status={decay} letztePartie={gesamt?.letzte_partie} className="id-card-decay-warn" iconSize={20} pulse />
      {onInvite && (
