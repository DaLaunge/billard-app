...
const TRANSLATIONS = {
  en: {
    ...
    "Durchschnittliche Zeit pro Einzelspiel bei 8-, 9- und 10-Ball-Matches mit gespeichertem Protokoll (nur Matches, die über den digitalen Zähler gemeldet wurden). Niedrigster Wert zuerst. Erst ab 3 Matches mit Protokoll gelistet, damit der Schnitt aussagekräftig ist.": "Average time per individual game in 8-, 9- and 10-ball matches with a saved protocol (only matches reported via the digital counter). Lowest value first. Only listed from 3 matches with a protocol onward, so the average is meaningful.",
    "Durchschnittliche Zeit pro versenkter Kugel bei 14/1-Endlos-Matches mit gespeichertem Protokoll. Fouls zählen nicht mit. Niedrigster Wert zuerst. Erst ab 3 Matches mit Protokoll gelistet, damit der Schnitt aussagekräftig ist.": "Average time per potted ball in 14.1 continuous matches with a saved protocol. Fouls don't count. Lowest value first. Only listed from 3 matches with a protocol onward, so the average is meaningful.",
