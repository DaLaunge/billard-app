...
const TRANSLATIONS = {
  en: {
    ...
    "Aktuelle Bestwerte der gesamten Gruppe aus allen bestätigten Einzel-Matches (bzw. dem bisherigen Rating-Verlauf beim Rating-Rekord) - wer hält gerade welchen Rekord? Schnellstes/Längstes Match zählen nur Matches mit gespeichertem Zeit-Protokoll (über den digitalen Zähler gemeldet) und sind unabhängig von der Disziplin-Auswahl oben, genau wie alle anderen Rekorde dieser Karte. Höchster Sieg gibt es getrennt für 14/1 (zählt in Punkten statt Racks, daher meist viel höhere Zahlen) und alle anderen Disziplinen.": "Current bests for the whole group across all confirmed singles matches (or the rating history so far for the rating record) - who currently holds which record? Fastest/longest match only count matches with a saved time protocol (reported via the digital counter) and are independent of the discipline selection above, just like every other record on this card. Biggest win is split into 14.1 (counted in points instead of racks, so usually much higher numbers) and all other disciplines.",
    "Übersicht": "Overview",
