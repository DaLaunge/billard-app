...
- **Re-numbering rows under a `UNIQUE(parent_id, position)` constraint in Postgres must never let two rows share a position "in transit".** Postgres checks non-deferred unique constraints at the end of each individual `UPDATE` statement, not at transaction end — so a single bulk `UPDATE ... SET position = position - 1 WHERE position > N` fails with `duplicate key value` if the row that used to hold position `N` hasn't been moved out of the way *first* (as a separate, prior statement). Hit this in `winner_stays_report_game()` (`supabase/2026-09-09b_fix_winner_stays_rotation.sql`) rotating the loser to the back of the queue with 3+ participants; fixed by moving the vacating row to a guaranteed-unused sentinel value (`-1`) before shifting everyone else down. Applies to any future "reorder a ranked/queued list" RPC, not just this one.
