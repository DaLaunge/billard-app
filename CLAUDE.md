...
**PWA/service worker update handling** is in App.jsx via `useRegisterSW`. Update checks run on an interval or on visibility change (configurable by the user, persisted to `localStorage`). A found update is **never** applied immediately — it's deferred to whichever of two triggers comes first, so users notice as little as possible:
1. **The app going to the background** (`visibilitychange` → hidden) — invisible, since nobody is looking at the screen. Skipped on `LIVE_ENTRY_TABS` (currently `match`, `winnerstays`) because their score/run-log state lives only in memory and would be lost even though nothing is visibly disrupted. **When adding a new screen that holds unconfirmed live entry across renders (a new live-scoring mode, an unsaved multi-step form, etc.), add its tab to `LIVE_ENTRY_TABS`** — Claude should proactively judge whether a new screen needs this and add it, or ask if unsure, without waiting to be told.
2. **The next real screen change** (hooked into `applyNavState`, the single function every navigation path — `navPush`, `navReplace`, browser back/forward — goes through). The reload rides along with the transition the user already triggered, so it's masked rather than a surprise. This needs **no tab list and no maintenance when new screens are added** — at a transition boundary nothing is ever "mid-entry", so it's inherently safe for any current or future screen (this is what protects a permanently-displayed tournament bracket screen from flickering while someone is watching it, while still allowing the update through the next time someone navigates within that section).

Either trigger persists the current navigation state (tab + ids like `tournamentId`/`winnerStaysId`/`protokollMatch`/etc.) to `sessionStorage` (`RESUME_NAV_KEY`) right before reloading, and restores it on the next mount (`resumedNav`) instead of falling back to the default start tab — otherwise a reload would silently bounce a permanently-displayed screen back to the home tab. See the comment block above `LIVE_ENTRY_TABS` near the top of [src/App.jsx](src/App.jsx) before changing this logic.
